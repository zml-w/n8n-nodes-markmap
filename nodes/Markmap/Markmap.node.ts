...
function buildHtmlWithCDN(
	...
	tableSpacing?: { tableCellPadding?: string; tableMargin?: string },
): string {
	const tableCellPadding = tableSpacing?.tableCellPadding || '4px 8px';
	const tableMargin = tableSpacing?.tableMargin || '8px 0';
			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						name: 'nodeSpacing',
						type: 'collection',
						placeholder: 'Add Spacing Option',
						default: {},
						description: 'Configure spacing between nodes',
						options: [
							{
							...
							{
								displayName: 'Node Min Height',
								name: 'nodeMinHeight',
								type: 'number',
								default: 16,
								description: 'Minimum height of each node in pixels',
							},
							{
								...
								type: 'number',
								default: 8,
								description: 'Horizontal padding inside nodes in pixels',
							},
						],
					},
					{
						displayName: 'Table Spacing',
						name: 'tableSpacing',
						type: 'collection',
						placeholder: 'Add Table Spacing Option',
						default: {},
						description: 'Configure spacing for tables in mindmap',
						options: [
							{
								displayName: 'Table Cell Padding',
								name: 'tableCellPadding',
								type: 'string',
								default: '4px 8px',
								description: 'Padding inside table cells (CSS format, e.g., "4px 8px")',
							},
							{
								displayName: 'Table Margin',
								name: 'tableMargin',
								type: 'string',
								default: '8px 0',
								description: 'Margin around tables (CSS format, e.g., "8px 0")',
							},
						],
					},
				],
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const renderMode = options.renderMode || 'cdn';
					...
					// Handle node spacing options
					if (options.nodeSpacing) {
						const { spacingHorizontal, spacingVertical, nodeMinHeight, paddingX } = options.nodeSpacing;
						if (spacingHorizontal !== undefined && spacingHorizontal !== 80) {
							jsonOptions.spacingHorizontal = spacingHorizontal;
						}
						if (spacingVertical !== undefined && spacingVertical !== 5) {
							jsonOptions.spacingVertical = spacingVertical;
						}
						if (nodeMinHeight !== undefined && nodeMinHeight !== 16) {
							jsonOptions.nodeMinHeight = nodeMinHeight;
						}
						if (paddingX !== undefined && paddingX !== 8) {
							jsonOptions.paddingX = paddingX;
						}
					}
					...
					const tableSpacing = options.tableSpacing;
					const html = renderMode === 'local'
						? buildHtmlWithLocal(rootJson, title, jsonOptionsJson, colorFreezeLevel, tableSpacing)
						: buildHtmlWithCDN(rootJson, title, jsonOptionsJson, colorFreezeLevel, tableSpacing);
